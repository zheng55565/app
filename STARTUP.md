...
| 组件 | 位置 | 技术栈 | 端口/入口 |
|---|---|---|---|
| 后端 API | `D:\new-api-app\server` | Node ≥18 + Express + pg | `http://localhost:3000`，健康检查 `GET /healthz` |
...
- App 的后端地址在 `app/lib/config.dart`：默认 `http://10.0.2.2:3000`（模拟器内访问宿主机的固定 IP），可用 `--dart-define=API_BASE_URL=...` 覆盖。
...
| 模拟器视角的宿主机 | `10.0.2.2`（后端即 `10.0.2.2:3000`，Clash 即 `10.0.2.2:7897`） |
...
2. **模拟器必须在清空代理环境变量后启动**（bash 用 `env -u`，cmd 用 `set HTTP_PROXY=`）。QEMU 会把代理变量应用到整个虚拟机，导致 App 连不上 `10.0.2.2:3000`。远程桌面下还必须加 `-gpu swiftshader_indirect` 软渲染，否则黑屏。
3. **模拟器每次开机后必须重做两条 adb 配置**（start-dev.bat 第 5 步自动做）：
   ...
   adb reverse tcp:3000 tcp:3000                             # OAuth 回调的 localhost:3000 在模拟器内可达
   ```
...
| 检查 | 命令 | 期望 |
|---|---|---|
| 后端存活 | `curl --noproxy '*' -s http://127.0.0.1:3000/healthz` | `{"ok":true}` |
| 数据库端口 | `netstat -ano \| grep 5433 \| grep LISTEN` | 有输出 |
| 模拟器在线 | `adb devices` | `emulator-5554  device` |
| 回调通道 | `adb reverse --list` | 含 `tcp:3000` |
...
先 `adb emu kill` 优雅关模拟器，再关 api-server / dev-db 窗口，最后兜底清理 3000/5433 端口残留进程。数据库数据（`server/.pgdata`）不会丢。
